...
const FormEditPanen = (props) => {
  ...
  const getData = async () => {
    ...
    const idPanen = await AsyncStorage.getItem('idPanen');
    const config = {
    ...
    try {
      const res = await Axios.get(
        'https://e-chick-backend.herokuapp.com/api/panen/find/' +
        idPanen,
        config,
      );
      setTanggal(JSON.stringify(res.data.data.tanggal))
      setPenerima(res.data.data.penerima)
    ...
    if (tanggal && keranjang) {
      ...
      try {
        ...
        const res = await Axios.put(
          'https://e-chick-backend.herokuapp.com/api/panen/update/' + idPanen,
          body,
          config,
        );
        setIsLoading(false);
  ...
  useEffect(() => {
    ...
    isSuccess == 'error' && alert('Update Panen Gagal');
  }, [isSuccess]);
    ...
    <ScrollView>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <Text style={styles.text_header}>Form untuk Update Panen</Text>
        </View>
        <Animatable.View
